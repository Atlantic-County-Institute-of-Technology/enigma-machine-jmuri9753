...
Have a "Main Menu" which offers the following options:
- [ ] Have the user write a message, and ask for a key. Encode the message using a *Vigenère cipher*, then display the encoded message to the screen and give the user the option to write to a new file.
- [ ] Have the user enter a filename containing a string message, and ask for a key. Encode the contents of the file using a *Vigenère cipher*, then display the encoded message, and ask the user if they'd like to **overwrite** the existing file with the new message, or write to a new file.
- [ ] Have the user enter a filename containing an encoded string message, and ask for a key. Decode the contents of the file using a *Vigenère cipher*, and ask the user if they'd like to **overwrite** the existing file with a new message, or write to a new file.
- [ ] Your program should be able to account for various input validation errors, and handle the input of numbers/symbols gracefully. 
- [ ] Your program should account for file system errors, such as *FileNotFound* exceptions.
